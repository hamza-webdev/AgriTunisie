...
exports.createParcelle = async (req, res, next) => {
    ...
    const { nom_parcelle, description, geometrie, superficie_calculee_ha, type_sol_predominant, culture_actuelle_id } = req.body;
    ...
    if (!nom_parcelle || !geometrie) {
        return res.status(400).json({ message: "Le nom de la parcelle et la géométrie sont requis." });
    }
    ...
    try {
        ...
        if (error.code === 'XX000' && error.message.includes('invalid GeoJSON representation')) {
             return res.status(400).json({ message: "Format GeoJSON invalide pour la géométrie."});
        }
        next(error);
    }
};
...
exports.getParcelleById = async (req, res, next) => {
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Parcelle non trouvée ou accès non autorisé." });
        }
...
exports.updateParcelle = async (req, res, next) => {
    ...
    if (!nom_parcelle) { // Exemple minimal
        return res.status(400).json({ message: "Le nom de la parcelle est requis." });
    }
    ...
    try {
        ...
        const result = await db.query(query, values);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Parcelle non trouvée ou accès non autorisé pour la mise à jour." });
        }
        res.status(200).json(result.rows[0]);
    } catch (error) {
...
exports.deleteParcelle = async (req, res, next) => {
    ...
    try {
        ...
        const result = await db.query(query, [id, utilisateur_id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Parcelle non trouvée ou accès non autorisé pour la suppression." });
        }
        res.status(200).json({ message: "Parcelle supprimée avec succès.", id: result.rows[0].id });
    } catch (error) {
