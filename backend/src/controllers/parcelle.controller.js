...
exports.getUserParcelles = async (req, res, next) => {
    ...
    try {
        const result = await parcelleService.findByUserId(utilisateur_id, { page, limit });
...
exports.getParcelleById = async (req, res, next) => {
    const { id } = req.params; // L'ID est déjà validé (entier) par le middleware
    const utilisateur_id = req.user.id;
...
exports.updateParcelle = async (req, res, next) => {
    ...
    try {
        ...
        if (!parcelleMiseAJour) { // Redondant si vérifié avant, mais bonne sécurité
            return res.status(404).json({ message: "Mise à jour échouée, parcelle non trouvée ou accès non autorisé." });
        }
