...
class GeminiService {
    constructor() {
        ...
        if (GEMINI_API_KEY) {
            try {
                ...
                this.model = this.genAI.getGenerativeModel({ model: "gemini-1.5-flash-latest" }); // ou "gemini-pro"
    ...
    async _logInteraction(utilisateur_id, type_requete, prompt, reponse, paramsEntree, succes, erreurMsg = null, dureeMs = null) {
        try {
            await dbGeminiLog.query(
                `INSERT INTO logs_interactions_gemini 
                ...
                [utilisateur_id, type_requete, prompt ? prompt.substring(0, 5000) : null, reponse ? JSON.stringify(reponse).substring(0, 5000) : null, paramsEntree, succes, erreurMsg, dureeMs]
            );
        } catch (logError) {
    ...
    async getConseilCulture(utilisateur_id, donneesParcelle, cultureInfo, historiqueMeteoLocal) {
        ...
        if (!this.model) {
            ...
            await this._logInteraction(utilisateur_id, 'conseil_culture', prompt, { conseil: "Simulation: Vérifiez le pH du sol." }, { donneesParcelle, cultureInfo }, true, null, Date.now() - startTime);
            return { conseil: "Conseil de culture simulé par l'IA : Assurez un bon drainage et une fertilisation NPK équilibrée. Pensez à la rotation des cultures." };
        }
        ...
        try {
            ...
            await this._logInteraction(utilisateur_id, 'conseil_culture', prompt, { conseil: text }, { donneesParcelle, cultureInfo }, true, null, Date.now() - startTime);
            return { conseil: text };
        } catch (error) {
            console.error("Erreur API Gemini (conseil culture):", error);
            await this._logInteraction(utilisateur_id, 'conseil_culture', prompt, null, { donneesParcelle, cultureInfo }, false, error.message, Date.now() - startTime);
    ...
    async optimiserRationAnimale(utilisateur_id, typeAnimal, alimentsDisponibles, stadeProduction, objectifs = "maintien et production standard") {
        ...
        if (!this.model) {
            console.warn("GeminiService (optimiserRation): Mode simulé activé.");
             await this._logInteraction(utilisateur_id, 'optimiser_ration', prompt, { ration: "Simulation: Foin à volonté, complémentation modérée." }, { typeAnimal, alimentsDisponibles, stadeProduction }, true, null, Date.now() - startTime);
            return { ration_suggeree: "Ration optimisée simulée : 60% fourrage de bonne qualité, 30% concentré énergétique et protéique, 10% compléments minéraux et vitaminiques. Eau propre et fraîche à volonté." };
        }
        try {
            const result = await this.model.generateContent(prompt);
            const response = await result.response;
            const text = response.text();
            await this._logInteraction(utilisateur_id, 'optimiser_ration', prompt, { ration_suggeree: text }, { typeAnimal, alimentsDisponibles, stadeProduction }, true, null, Date.now() - startTime);
            return { ration_suggeree: text };
        } catch (error) {
            console.error("Erreur API Gemini (optimiser ration):", error);
            await this._logInteraction(utilisateur_id, 'optimiser_ration', prompt, null, { typeAnimal, alimentsDisponibles, stadeProduction }, false, error.message, Date.now() - startTime);
            throw { status: 500, message: "Erreur lors de l'optimisation de la ration par l'IA." };
        }
    }
    ...
    async resumerTexteActualite(utilisateur_id, texte) {
        ...
        if (!texte || texte.trim().length < 50) {
             await this._logInteraction(utilisateur_id, 'resumer_actualite', texte, null, { texteLength: texte?.length }, false, "Texte trop court.", Date.now() - startTime);
        ...
        if (!this.model) {
            ...
            await this._logInteraction(utilisateur_id, 'resumer_actualite', prompt, { resume: "Simulation: Nouvelles mesures gouvernementales impactant les subventions." }, { texteLength: texte.length }, true, null, Date.now() - startTime);
            return { resume: "Résumé simulé: 1. Annonce de nouvelles subventions pour les semences certifiées. 2. Rappel des dates limites pour les déclarations de récolte. 3. Alerte sur une nouvelle maladie affectant les agrumes dans certaines régions."};
        }
        try {
            const result = await this.model.generateContent(prompt);
            const responseText = result.response.text();
            await this._logInteraction(utilisateur_id, 'resumer_actualite', prompt, { resume: responseText }, { texteLength: texte.length }, true, null, Date.now() - startTime);
            return { resume: responseText };
        } catch (error) {
            console.error("Erreur API Gemini (résumé actualité):", error);
            await this._logInteraction(utilisateur_id, 'resumer_actualite', prompt, null, { texteLength: texte.length }, false, error.message, Date.now() - startTime);
            throw { status: 500, message: "Erreur lors du résumé du texte par l'IA." };
        }
    }
}
