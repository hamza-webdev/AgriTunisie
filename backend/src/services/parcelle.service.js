...
class ParcelleService {
    async create(utilisateur_id, data) {
        const { nom_parcelle, description, geometrie, superficie_calculee_ha, type_sol_predominant, culture_actuelle_id } = data;
        ...
        try {
            ...
            if (error.code === 'XX000' && error.message.includes('invalid GeoJSON representation')) {
                 throw { status: 400, message: "Format GeoJSON invalide pour la géométrie."};
            }
            throw error; // Relancer pour gestion globale
        }
    }

    async findByUserId(utilisateur_id, { page = 1, limit = 10 }) {
        ...
        try {
            const [dataResult, countResult] = await Promise.all([
                db.query(dataQuery, [utilisateur_id, limit, offset]),
                db.query(countQuery, [utilisateur_id])
            ]);
    ...
    async update(id, utilisateur_id, data) {
        ...
        try {
            ...
            return result.rows[0];
             ...
             if (error.code === 'XX000' && error.message.includes('invalid GeoJSON representation')) {
                 throw { status: 400, message: "Format GeoJSON invalide pour la géométrie lors de la mise à jour."};
            }
            throw error;
        }
    }
