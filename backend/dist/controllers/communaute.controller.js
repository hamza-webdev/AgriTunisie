...
exports.getCategories = async (req, res, next) => {
  ...
  try {
    res.json(await communauteServiceInstance.getCategories({
...
exports.createPost = async (req, res, next) => {
  try {
    res.status(201).json(await communauteServiceInstance.createPost(req.user.id, req.body));
  } catch (e) {
...
exports.getPosts = async (req, res, next) => {
  ...
  try {
    res.json(await communauteServiceInstance.getPostsByCategorie(categorieId, {
      page: parseInt(page) || 1,
...
exports.addCommentToPost = async (req, res, next) => {
  ...
  try {
    ...
    if (!postExists) return res.status(404).json({
      message: "Post non trouvé pour commenter."
    });
...
exports.getCommentsForPost = async (req, res, next) => {
  ...
  try {
    res.json(await communauteServiceInstance.getCommentsByPost(postId, {
      page: parseInt(page) || 1,
      limit: parseInt(limit) || 10
    }));
  } catch (e) {
