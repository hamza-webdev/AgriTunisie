...
exports.getTypesAnimaux = async (req, res, next) => {
  ...
  try {
    const result = await elevageServiceInstance.getTypesAnimaux({
...
exports.getAnimauxUtilisateur = async (req, res, next) => {
  ...
  try {
    const result = await elevageServiceInstance.getAnimauxUtilisateur(req.user.id, {
      page: parseInt(page) || 1,
      limit: parseInt(limit) || 10
    });
    res.status(200).json(result);
...
exports.getAnimalUtilisateur = async (req, res, next) => {
  try {
    ...
    if (!animal) return res.status(404).json({
      message: "Animal non trouvé ou accès non autorisé."
    });
...
exports.updateAnimalUtilisateur = async (req, res, next) => {
  try {
    ...
    if (!animal) return res.status(404).json({
      message: "Animal non trouvé ou accès non autorisé pour la mise à jour."
    });
    res.status(200).json(animal);
  } catch (error) {
    if (error.status === 400) return res.status(400).json({
      message: error.message
    });
    if (error.code === '23503') return res.status(400).json({
      message: "Type d'animal invalide pour la mise à jour."
    });
    next(error);
  }
};
exports.deleteAnimalUtilisateur = async (req, res, next) => {
  try {
    ...
    if (!deleted) return res.status(404).json({
      message: "Animal non trouvé ou accès non autorisé pour la suppression."
    });
    res.status(200).json({
