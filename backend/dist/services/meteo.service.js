...
class MeteoService {
  ...
  formatOpenWeatherForecast(apiResponse, units) {
    /* ... (comme précédemment) ... */
    ...
    apiResponse.list.forEach(item => {
      ...
      dailyForecasts[date].temps.push({
      ...
      if (item.rain && item.rain['3h']) {
        dailyForecasts[date].precipitations_3h.push(item.rain['3h']);
      } else if (item.snow && item.snow['3h']) {
        dailyForecasts[date].precipitations_3h.push(item.snow['3h']);
      }
    });
    const simplifiedForecast = Object.values(dailyForecasts).map(dayData => {
      const representativeWeather = dayData.temps.length > 0 ? dayData.temps[Math.floor(dayData.temps.length / 2)] : {
        description: 'N/A',
        icon: '01d'
      };
      ...
      return {
        date: dayData.date,
        temp_min: parseFloat(dayData.temp_min.toFixed(1)),
        temp_max: parseFloat(dayData.temp_max.toFixed(1)),
    ...
    return {
      message: "Prévisions météo récupérées.",
  ...
  getSimulatedPrevisions(latitude, longitude, units) {
    /* ... (comme précédemment) ... */
    return {
      message: "Prévisions simulées",
      city: "Simville",
      country: "TN",
      latitude,
      longitude,
  ...
  async getHistorique(latitude, longitude, dateStart, dateEnd) {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        return {
          message: "Historique depuis cache.",
          latitude,
          longitude,
          dateStart,
          dateEnd,
          data: result.rows.map(r => ({
            ...r,
            precipitation_total: r.precipitations_mm
          }))
        };
      } else {
        return {
