...
class PrixService {
   ...
   */
  async searchObservations(filters, {
    page = 1,
    limit = 10
  }) {
    const {
    ...
    try {
      const [dataResult, countResult] = await Promise.all([db.query(baseQuery, dataQueryParams), db.query(countBaseQuery, queryParams) // Utiliser queryParams sans limit/offset
      ]);
  ...
  async addObservation(data, user) {
    const {
      produit_id,
      region_id,
      nom_marche_specifique,
      prix_moyen_kg_ou_unite,
      unite_prix,
      date_observation,
      source_information_input
    } = data;
    ...
    try {
      ...
      if (error.code === '23503') {
        ...
        if (produitExists.rowCount === 0) message = "ID de produit invalide.";else if (regionExists.rowCount === 0) message = "ID de région invalide.";
        throw {
          status: 400,
          message
        };
