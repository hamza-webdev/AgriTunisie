...
class ElevageService {
  ...
  async getTypesAnimaux({
    ...
    try {
      ...
      console.error("Erreur ElevageService.getTypesAnimaux:", error);
      throw error;
    }
  }
  async addAnimalUtilisateur(utilisateur_id, data) {
    ...
    try {
      ...
      if (error.code === '23503') throw {
        status: 400,
        message: "Type d'animal invalide."
      };
      console.error("Erreur ElevageService.addAnimalUtilisateur:", error);
      throw error;
    }
  }
  async getAnimauxUtilisateur(utilisateur_id, {
    page = 1,
    limit = 10
  }) {
    const offset = (page - 1) * limit;
    ...
    try {
      ...
      console.error("Erreur ElevageService.getAnimauxUtilisateur:", error);
      throw error;
    }
  }
  async getAnimalUtilisateurById(id, utilisateur_id) {
    ...
    try {
      ...
      console.error("Erreur ElevageService.getAnimalUtilisateurById:", error);
      throw error;
    }
  }
  async updateAnimalUtilisateur(id, utilisateur_id, data) {
    /* ... (comme précédemment, attention aux index des paramètres) ... */
    const {
      animal_type_id,
      identifiant_animal,
      date_naissance_approx,
    ...
    if (animal_type_id !== undefined) {
      fields.push(`animal_type_id = $${paramCount++}`);
      values.push(animal_type_id);
    }
    if (identifiant_animal !== undefined) {
      fields.push(`identifiant_animal = $${paramCount++}`);
      values.push(identifiant_animal);
    }
    if (date_naissance_approx !== undefined) {
      fields.push(`date_naissance_approx = $${paramCount++}`);
      values.push(date_naissance_approx);
    }
    if (notes_sante !== undefined) {
      fields.push(`notes_sante = $${paramCount++}`);
      values.push(notes_sante);
    }
    ...
    try {
      ...
      if (error.code === '23503') throw {
        status: 400,
        message: "Type d'animal invalide pour MàJ."
      };
      console.error("Erreur ElevageService.updateAnimalUtilisateur:", error);
      throw error;
    }
  }
  async deleteAnimalUtilisateur(id, utilisateur_id) {
    ...
    try {
      ...
      console.error("Erreur ElevageService.deleteAnimalUtilisateur:", error);
      throw error;
    }
  }
}
