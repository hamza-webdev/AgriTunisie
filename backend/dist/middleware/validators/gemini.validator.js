...
const conseilCultureRules = () => [bodyGemini('donneesParcelle').notEmpty().isObject().withMessage("Données de parcelle requises et doivent être un objet."), bodyGemini('cultureInfo').notEmpty().isObject().withMessage("Informations sur la culture requises et doivent être un objet."), bodyGemini('historiqueMeteoLocal').optional().isObject()];
const optimiserRationRules = () => [bodyGemini('typeAnimal').notEmpty().isString(), bodyGemini('alimentsDisponibles').notEmpty().isArray({
  min: 1
}).withMessage("Liste d'aliments disponibles requise."), bodyGemini('stadeProduction').notEmpty().isString()];
const resumerActualiteRules = () => [bodyGemini('texte_actualite').trim().notEmpty().isLength({
  min: 50,
  max: 10000
}).withMessage("Texte d'actualité requis (50-10000 caractères).")];
const validateGemini = (req, res, next) => {
...
const {
  authenticateToken: authenticateTokenGemini
