  ...
  if (value.type === 'Polygon') {
    ...
    value.coordinates.forEach(ring => {
      if (!Array.isArray(ring) || ring.length < 4) {
        // Un ring doit avoir au moins 4 points (3 distincts + fermeture)
...
const parcelleValidationRules = () => {
  return [body('nom_parcelle').trim().notEmpty().withMessage('Le nom de la parcelle est requis.').isString().withMessage('Le nom de la parcelle doit être une chaîne de caractères.').isLength({
    min: 3,
    max: 255
  }).withMessage('Le nom de la parcelle doit contenir entre 3 et 255 caractères.'), body('description').optional({
    checkFalsy: true
  }) // Permet une chaîne vide ou null
  .trim().isString().withMessage('La description doit être une chaîne de caractères.'), body('geometrie').notEmpty().withMessage('La géométrie de la parcelle est requise.').custom(validateGeoJSON),
  // Validation personnalisée pour GeoJSON
  body('superficie_calculee_ha').optional({
    checkFalsy: true
  }).isNumeric().withMessage('La superficie doit être un nombre.').toFloat(), body('type_sol_predominant').optional({
    checkFalsy: true
  }).trim().isString().withMessage('Le type de sol doit être une chaîne de caractères.').isLength({
    max: 100
  }).withMessage('Le type de sol ne doit pas dépasser 100 caractères.'), body('culture_actuelle_id').optional({
    checkFalsy: true
  }).isInt({
    min: 1
  }).withMessage('L\'ID de la culture actuelle doit être un entier positif.').toInt()];
};
const parcelleIdValidationRules = () => {
  return [param('id').isInt({
    min: 1
  }).withMessage('L\'ID de la parcelle doit être un entier positif.').toInt()];
};
